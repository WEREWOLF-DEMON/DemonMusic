...
import config
from Demon import Apple, Resso, SoundCloud, Spotify, Telegram, YouTube, app
from Demon.core.call import DEMON
from Demon.misc import SUDOERS
from Demon.utils.inline import panel_markup_clone
from Demon.utils import seconds_to_min, time_to_seconds
from Demon.utils.channelplay import get_channeplayCB
from Demon.utils.decorators.language import languageCB
from Demon.utils.decorators.play import CPlayWrapper
from Demon.utils.formatters import formats
from Demon.utils.inline import (
    botplaylist_markup,
    livestream_markup,
    playlist_markup,
    slider_markup,
...
from Demon.utils.database import (
...
from Demon.utils.logger import play_logs, clone_bot_logs
from Demon.cplugin.setinfo import get_logging_status, get_log_channel
from config import BANNED_USERS, lyrical
from time import time
from Demon.utils.extraction import extract_user
        ...
        if await YouTube.exists(url):
            if "playlist" in url:
                ...
                try:
                    ...
                    print(e)  # Handle or log the error apDEMONpriately
...
import config
from Demon import Carbon, YouTube
from Demon.core.call import DEMON
from Demon.misc import db
from Demon.utils.database import add_active_video_chat, is_active_chat
from Demon.utils.exceptions import AssistantErr
from Demon.utils.inline import (
    aq_markup,
    queuemarkup,
    close_markup,
...
from Demon.utils.pastebin import DEMONBin
from Demon.utils.stream.queue import put_queue, put_queue_index
from youtubesearchpython.__future__ import VideosSearch
from Demon.utils.database.clonedb import get_owner_id_from_db, get_cloned_support_chat, get_cloned_support_channel
...
async def stream(
    ...
    if not result:
        return
    if forceplay:
        await DEMON.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await DEMON.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await DEMON.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
